...
tf.app.flags.DEFINE_integer('workers',5, '')
...
def average_gradients(tower_grads):
    '''
    average gradients from different towers(gpus)
    '''
    average_grads = []
        ...
        for epoch in range(FLAGS.epochs):
            for step in range(len(generator)):
                ...
                if len(data[0])%len(gpus)!=0:
                    temp_data = []
                    for item in data:
                        temp_data.append(item[:len(data[0])//len(gpus)*len(gpus)])       
                data = temp_data
                ...
                if step % step_print == 0:
                    ...
                    avg_examples_per_second = (step_print * FLAGS.batch_size)/(time.time() - start)
                    start = time.time()
                ...
                if step % FLAGS.save_summary_steps == 0:
                    _, tl, summary_str = sess.run([train_op, total_loss, summary_op],
                                                  feed_dict={input_images: data[0],                                                                                    input_score_maps: data[1],
                                                             input_geo_maps: data[2],
                                                             input_training_masks: data[3]})
